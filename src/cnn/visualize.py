# Shows a 3-panel demo: all keypoints | YOLO mask | filtered keypoints. Usage: python src/visualize.py --sequence 00
...
def extract_orb_keypoints(image_gray, n_features=1000):
...
def build_panel(image_path, mask=None, conf=0.4):
    ...
    all_keypoints, _ = extract_orb_keypoints(image_gray)
...
def run_sequence_sample(sequence_id, n_samples=5, save_dir=None, conf=0.4):
    seq_id    = sequence_id.zfill(2)
    image_dir = os.path.join(PROJECT_ROOT, 'data', 'sequences', seq_id, 'image_2')
    mask_dir  = os.path.join(PROJECT_ROOT, 'output', 'masks', seq_id)
...
if __name__ == '__main__':
    ...
    group.add_argument('--sequence', type=str, help='KITTI sequence ID, e.g. 00. Defaults to the first available sequence.')
    ...
    if args.image:
        ...
        run_sequence_sample(sequence, n_samples=args.sample, save_dir=args.save, conf=args.conf)
