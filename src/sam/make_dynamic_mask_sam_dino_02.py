...
def save_dino_detection_image(image_source, boxes, logits, phrases, output_path):
    """
    Save GroundingDINO detection result image:
    ...
    # annotate() output may be RGB depending on environment.
...
def main():
    ...
    # 1. GroundingDINO detection result
    dino_detection_path = f"{args.output_prefix}_dino_detection.png"
    save_dino_detection_image(
        image_source=image_source,
        boxes=boxes,
        logits=logits,
        phrases=phrases,
        output_path=dino_detection_path
    )

    # 2. Dynamic mask
    dynamic_mask_path = f"{args.output_prefix}_dynamic_mask.png"
    cv2.imwrite(dynamic_mask_path, dynamic_mask)

    # 3. Dynamic overlay
    dynamic_overlay_path = f"{args.output_prefix}_dynamic_overlay.png"
    overlay_mask(
        args.image_path,
        dynamic_mask,
    ...
    print("[DONE]")
    print(f"saved: {dino_detection_path}")
    print(f"saved: {dynamic_mask_path}")
    print(f"saved: {dynamic_overlay_path}")
    print(f"saved: {args.output_prefix}_orb_before.png")
    print(f"saved: {args.output_prefix}_orb_after.png")
    print(f"saved: {args.output_prefix}_static_mask.png")


if __name__ == "__main__":
    main()

#% python make_dynamic_mask_sam_dino_02.py \
